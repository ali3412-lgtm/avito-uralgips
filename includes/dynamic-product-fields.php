 ...
 * Все поля Avito отображаются в отдельной вкладке "Avito" в блоке данных товара
 *
...
/**
 * Добавляем вкладку "Avito" в блок данных товара
 */
add_filter('woocommerce_product_data_tabs', 'wc_avito_add_product_data_tab');

function wc_avito_add_product_data_tab($tabs) {
    $tabs['avito'] = array(
        'label'    => 'Avito',
        'target'   => 'avito_product_data',
        'class'    => array(),
        'priority' => 80,
    );
    return $tabs;
...
/**
 * Добавляем иконку для вкладки Avito
 */
add_action('admin_head', 'wc_avito_add_tab_icon');

function wc_avito_add_tab_icon() {
    ?>
    <style>
        #woocommerce-product-data ul.wc-tabs li.avito_options a::before {
            content: "\f173";
...
/**
 * Добавляем содержимое вкладки Avito
 */
add_action('woocommerce_product_data_panels', 'wc_avito_add_product_data_panel');

function wc_avito_add_product_data_panel() {
    global $post;
    ?>
    <div id="avito_product_data" class="panel woocommerce_options_panel">
        <div class="options_group">
            <h4 style="padding-left: 12px; margin-bottom: 0;">Настройки экспорта</h4>
        ...
        if (!empty($settings['product_fields'])) {
            ...
            echo '<h4 style="padding-left: 12px; margin-bottom: 0;">Поля объявления</h4>';

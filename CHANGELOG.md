...
### Исправления
- **Исправлена генерация вложенных XML структур для nested полей**
  - Исправлена проблема, когда верхний уровень JSON не создавал соответствующий XML элемент
  - Теперь `{"Item": {"Region": "Ижевск", "Bid": "10"}}` корректно генерирует `<Item><Region>Ижевск</Region><Bid>10</Bid></Item>`
  - Исправление применено для всех типов полей: глобальных, категорий и товаров
  - До исправления: элементы верхнего уровня JSON пропускались
  - После исправления: все уровни вложенности обрабатываются корректно

### Технические детали
- Изменена логика обработки JSON в функциях генерации XML
- Теперь для каждого элемента верхнего уровня JSON создается соответствующий XML элемент
- Сохранена поддержка рекурсивной обработки для глубоко вложенных структур

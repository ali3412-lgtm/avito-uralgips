...
define('WC_AVITO_VDOM_VERSION', '1.1.1');
...
require_once WC_AVITO_VDOM_PLUGIN_DIR . 'includes/field-manager.php';
require_once WC_AVITO_VDOM_PLUGIN_DIR . 'includes/field-settings-page.php';
require_once WC_AVITO_VDOM_PLUGIN_DIR . 'includes/xml-generator-dynamic.php';
require_once WC_AVITO_VDOM_PLUGIN_DIR . 'includes/admin-menu.php';
// Поле экспорта для категорий (контролирует экспорт всех товаров категории)
...
function wc_avito_vdom_activate() {
    ...
    // Инициализация настроек полей XML (если еще не созданы)
    if (!get_option('wc_avito_field_settings')) {
        // Получаем дефолтные настройки полей из field-manager.php
        require_once WC_AVITO_VDOM_PLUGIN_DIR . 'includes/field-manager.php';
        $default_settings = wc_avito_get_default_field_settings();
        add_option('wc_avito_field_settings', $default_settings);
    }
